...
Once the Trusted Publisher exists and `master` is green, create a published GitHub Release. This command creates the `v3.0.0` tag from `master` and publishes the release:
...
python -m pip install --upgrade custom-dl-optimizer==3.0.0
...
python -m build
python -m twine check dist/*
python -m twine upload --verbose dist/custom_dl_optimizer-3.0.0*
```
